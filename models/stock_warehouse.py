...
class StockWarehouse(models.Model):
    ...
    mts_mto_diff_rule_id = fields.Many2one("stock.rule", "MTO+MTS diff rule", check_company=True)

    def _get_all_routes(self):
        routes = super(StockWarehouse, self)._get_all_routes()
        routes |= self.mapped("mts_mto_diff_rule_id.route_id")
        return routes
    ...
    def _update_name_and_code(self, new_name=False, new_code=False):
        ...
        if not new_name:
            return res
        for warehouse in self.filtered("mts_mto_diff_rule_id"):
            warehouse.mts_mto_diff_rule_id.write(
                {
                    "name": warehouse.mts_mto_diff_rule_id.name.replace(
    ...
    def _get_route_name(self, route_type):
        if route_type == "mts_mto_diff":
            return _("MTS+MTO Diff")
                ...
                "mts_mto_diff_rule_id": {
                    ...
                    "create_values": {
                        ...
                        "route_id": self._find_global_route(
                            "stock_mts_mto_diff_rule.route_mto_mts_diff",
                            _("Make To Order + Make To Stock (Diff)"),
                        ).id,
                    },
                    "update_values": {
                        "active": self.mto_mts_management,
                        "name": self._format_rulename(
                            location_id, location_dest_id, "MTS+MTO Diff" 
                        ),
    ...
    def _create_or_update_global_routes_rules(self):
        ...
        if (
            ...
            and self.mts_mto_diff_rule_id
            and self.mts_mto_diff_rule_id.action != "split_procurement_diff" 
        ):
            # Cannot create or update with the 'split_procurement_diff' action due
            ...
            rule = self.env["stock.rule"].search(
                [
                    ("location_dest_id", "=", self.mts_mto_diff_rule_id.location_dest_id.id),
                    ("location_src_id", "=", self.mts_mto_diff_rule_id.location_src_id.id),
                    ("route_id", "=", self.delivery_route_id.id),
                ],
                limit=1,
            )
            self.mts_mto_diff_rule_id.write(
                {
                    "action": "split_procurement_diff",
                    "mts_rule_id": rule.id,
