...
class StockRule(models.Model):
    ...
    action = fields.Selection(
        selection_add=[("split_procurement_diff", "Choose between MTS and MTO (Diff)")],
        ondelete={"split_procurement_diff": "cascade"},
    )
    ...
    def _check_mts_mto_rule(self):
        for rule in self:
            if rule.action == "split_procurement_diff":
    ...
    def get_mto_qty_to_order(self, product, product_qty, product_uom, values):
        """Calculate the qty to order via MTO: Ordered - Qty_available (if not enough stock).""" 
        self.ensure_one()
        ...
        if float_compare(qty_available, 0.0, precision_digits=precision) > 0:
            if (
                ...
                return product_qty - qty_available  # The difference formula as requested
        return product_qty

    def _run_split_procurement_diff(self, procurements):
        precision = self.env["decimal.precision"].precision_get(
            "Product Unit of Measure" 
        )
